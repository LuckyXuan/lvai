...
var ablumElement = function(){
	...
	this.ablum_li = null;
	//1初始化控件
	this.initAblumObj = function(){
		that.ablum_li = document.createElement('li');
		that.ablum_li.className ="mui-table-view-cell mui-media mui-col-xs-6";
		that.ablum_li.id = "ablum_"+that.id;
		...
		s+='</a>';
		that.ablum_li.innerHTML = s;
		
	}
	
	//2设置事件
	this.setFun = function(h,fun){
		that.ablum_li.addEventListener(h,fun);
	}
	
	
	//3获取一个相册
	this.getAblumObj = function(){
		return that.ablum_li;
	}
	//获取图片数量
